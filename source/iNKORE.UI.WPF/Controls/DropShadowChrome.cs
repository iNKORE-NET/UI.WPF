...
namespace iNKORE.UI.WPF.Controls
{
    public sealed class DropShadowChrome : Decorator
    {
        ...
        /// <summary>
        /// DependencyProperty for <see cref="ShadowDepth" /> property.
        /// </summary>
        ...
        private GradientStopCollection CreateStops(Color c, double cornerRadius)
        {
            ...
            double gradientScale = 1 / (cornerRadius + BlurRadius);
            ...
            //gsc.Add(new GradientStop(stopColor, (BlurRadius * 0.3 + cornerRadius) * gradientScale));

            //stopColor.A = (byte)(.38053 * c.A);
            //gsc.Add(new GradientStop(stopColor, (BlurRadius * 0.5 + cornerRadius) * gradientScale));

            //stopColor.A = (byte)(.12389 * c.A);
            //gsc.Add(new GradientStop(stopColor, (BlurRadius * 0.7 + cornerRadius) * gradientScale));

            //stopColor.A = (byte)(.02654 * c.A);
            //gsc.Add(new GradientStop(stopColor, (BlurRadius * 0.9 + cornerRadius) * gradientScale));

            //stopColor.A = 0;
            //gsc.Add(new GradientStop(stopColor, (BlurRadius * 1 + cornerRadius) * gradientScale));

            stopColor.A = c.A;
            gsc.Add(new GradientStop(stopColor, 0));
